#+TITLE: Be careful when writing assembly
#+INCLUDE: back.org
...
在如下这段程序中，caller使用了 =%rbx= ，而atomic_load_u128改变了它，导致程序出错。
...
图中， =scq_push(scq_t *q, u64 val)= 传入的q是一个有效的指针，且scq\under{}push没有对q做过任何修改，在执行到149行后，q变为了0x0，像幻觉一样。这是因为在 =movq %rcx, %rbx= 时，修改了 =rbx= ，而 =rcx= 存的是 =new.data=val=0= ，因此，在返回后，q变为了0。
...
你可以选择使用内存屏障(如mfence，std::memory\under{}order)、原子操作(如gcc内置的\under{}\under{}sync\under{}\under{})、锁（如spinlock，linux futex）等来同步你的线程对于共享变量的访问。 从左到右序依次增强、同步逻辑依次简化，但开销也依次增大。
